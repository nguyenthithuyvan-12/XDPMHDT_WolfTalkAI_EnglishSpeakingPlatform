...
export const PlansPage: React.FC = () => {
  const { plans, loading, error, createPlan, updatePlan, deletePlan, togglePlanStatus } =
    usePlans();

  const [searchQuery, setSearchQuery] = useState('');
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [selectedPlan, setSelectedPlan] = useState<PlanDTO | null>(null);
  const [formData, setFormData] = useState<CreatePlanDTO & { isActive?: boolean }>({
    name: '',
    description: '',
    price: 0,
    currency: 'VND',
  ...
  const openEditModal = (plan: PlanDTO) => {
    ...
    setFeatures(plan.features.map(f => ({ name: f.name, included: f.included })));
    setIsCreateModalOpen(true);
  };
  ...
  const handleFeatureChange = (index: number, field: keyof PlanFeature, value: string | boolean) => {
    ...
    <div className="plans-page">
      ...
      <div className="plans-page__header">
        <div className="plans-page__header-main">
          ...
          <div className="plans-page__header-content">
            ...
            <p className="plans-page__subtitle">Manage your pricing tiers and features</p>
          </div>
        </div>
      ...
      <div className="plans-page__controls">
        <div className="plans-page__search">
          <Search className="plans-page__search-icon" size={20} strokeWidth={2} />
          <input
            type="text"
      ...
      {filteredPlans.length > 0 ? (
        ...
        <div className="plans-page__empty">
          <Package className="plans-page__empty-icon" size={64} strokeWidth={1.5} />
      ...
      >
        <form className="plans-page__form" onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          {/* Name */}
          <Input
            label="Plan Name"
            value={formData.name}
    ...
    >
      ...
      <div className="plans-page__pricing">
        <div className="plans-page__price-wrapper">
          ...
          <span className="plans-page__price">{formatCurrency(plan.price, plan.currency)}</span>
        </div>
      ...
      <div className="plans-page__features">
        ...
        <ul className="plans-page__features-list">
          {plan.features.map((feature, idx) => (
            ...
            >
              {feature.included ? (
                <Check size={16} strokeWidth={2.5} className="plans-page__feature-icon plans-page__feature-icon--check" />
              ) : (
                <X size={16} strokeWidth={2.5} className="plans-page__feature-icon plans-page__feature-icon--x" />
              )}
          ...
          {plan.hasMentor && (
            <li className="plans-page__feature-item plans-page__feature-item--mentor">
              <Award size={16} strokeWidth={2.5} className="plans-page__feature-icon plans-page__feature-icon--award" />
      ...
      <div className="plans-page__actions">
        <button className="plans-page__action-btn plans-page__action-btn--primary" onClick={onEdit}>
        ...
        <button className="plans-page__action-btn plans-page__action-btn--danger" onClick={onDelete}>
          <Trash2 size={16} strokeWidth={2} />
          <span>Delete</span>
        </button>
      </div>
